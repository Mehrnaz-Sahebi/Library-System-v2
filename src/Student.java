...
public class Student extends User implements Borrower ,CommentWriter{
    ...
    public boolean canBorrow(String libraryId, Source source, String date, String time){
        if(getCountOfBorrowedSources()>=MAX_SOURCES_STUDENT_CAN_BORROW||getDebt()>0){
            return false;
        }
    ...
    public long returns(String libraryId, Source source, String date, String time){
        ...
        if(source instanceof NormalBook && differenceOFDays<=MAX_HOURS_BOOK_STUDENT){
            ...
            long newDebt = (differenceOFDays-MAX_HOURS_BOOK_STUDENT)*STUDENT_PENALTY_FOR_AN_HOUR;
            addToDebt(newDebt);
            return newDebt;
        } else if (source instanceof Thesis && differenceOFDays<=MAX_HOURS_THESIS_STUDENT){
            return 0;
        } else{// if(source instanceof Thesis && differenceOFDays>MAX_HOURS_THESIS_STUDENT){
            long newDebt = (differenceOFDays-MAX_HOURS_THESIS_STUDENT)*STUDENT_PENALTY_FOR_AN_HOUR;
            addToDebt(newDebt);
            return newDebt;
        }
    }
